...
import akonix.akonixboss.entity.ai.StationaryAttack;
...
public class RosalyneEntity extends BossEntity implements PowerableMob {
	...
	public static void spawn(Player player, Level world) {
		...
		RosalyneEntity dame = MYFEntities.THE_JOCKPAD.get().create(world);
	...
	private void createSpirits() {
		for (int i = 0; i < 4; i++) {
			RoseSpiritEntity spirit = MYFEntities.JOCKPAD_SPIRIT.get().create(level());
	...
	public void swing() {
		...
		playSound(MYFSounds.theJockpadSwing.get(), 1, 1);
	...
	protected SoundEvent getHurtSound(DamageSource damageSourceIn) {
		return MYFSounds.theJockpadHurt.get();
	}

	@Override
	protected SoundEvent getDeathSound() {
		return MYFSounds.theJockpadDeath.get();
	}

	@Override
	protected SoundEvent getMusic() {
		return MYFSounds.musicTheJockpad.get();
	}

	@Override
	protected ResourceLocation getDefaultLootTable() {
		return AkonixBoss.rl("the_jockpad");
	}
			...
			if (timer <= DURATION-30 && timer % 20 == 10) {
				switch (rosalyne.phase) {
					case BREAKING_OUT:
						...
						break;
					case MADDENING:
						rosalyne.playSound(MYFSounds.theJockpadCrack.get());
						break;
				}
			}
			if (timer <= 0) {
		...
		public void tick() {
			...
			//Circling
				...
				if (timer <= 0) {
					...
					rosalyne.setAnimation(ANIM_ARM_OUT_DN);
					rosalyne.playSound(MYFSounds.theJockpadSwingPrepare.get(), 1, 1);
				}
			}

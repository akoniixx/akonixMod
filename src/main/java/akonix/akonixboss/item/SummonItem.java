...
public class SummonItem extends Item {
	...
	public ItemStack finishUsingItem(ItemStack stack, Level world, LivingEntity entityLiving) {
		...
		if (!world.isClientSide) {
			if (!world.getEntitiesOfClass(Mob.class, player.getBoundingBox().inflate(32), e -> e.getType().is(Tags.EntityTypes.BOSSES) && e.isAlive()).isEmpty()) {
				player.displayClientMessage(Component.translatable("status.akonixboss .boss_nearby"), true);
				return stack;
			}
	...
	public void appendHoverText(ItemStack stack, Level worldIn, List<Component> tooltip, TooltipFlag flagIn) {
		...
		tooltip.add(Component.translatable("item.akonixboss .summon.use").withStyle(ChatFormatting.GRAY));
		tooltip.add(Component.translatable("item.akonixboss .summon.gear", Component.translatable(getDescriptionId() + ".gear").withStyle(ChatFormatting.GRAY)).withStyle(ChatFormatting.GOLD));
	}
